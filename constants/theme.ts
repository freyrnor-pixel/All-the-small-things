...
function buildCustomTheme(primary: string, secondary: string, isDark: boolean): AppColors {
  if (isDark) {
    return {
      ...
      // Navy-based depth ladder: cream → white → offWhite in ~10% steps,
      // all staying within the dark family for cohesion.
      white: '#121220',
      offWhite: '#0A0A12',
      gray: '#7A7A98',
      grayLight: '#161628',
...
export const DARK_THEMES: Record<ThemeName, AppColors> = {
  default: {
    ...
    brown: '#93C5FD',
    brownLight: '#112040',
    // Navy-based depth ladder: cream → white → offWhite each steps up in lightness,
    // all staying within the navy family so they read as cohesive surfaces.
    white: '#0F1829',
    offWhite: '#060914',
    gray: '#57789A',
    grayLight: '#131E30',
    ...
    // Subtle inky border, not the bright primary blue — every card border
    // should be understated, not glowing bright.
    border: '#1A2B44',
    neutral: '#4A6882',
    hintBg: '#0C1E38',
    hintBorder: '#1A3A60',
