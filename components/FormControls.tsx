    ...
    >
      <View
        style={[
          styles.checkboxBox,
          {
            backgroundColor: checked ? theme.orange : 'transparent',
            borderColor: checked ? theme.orange : theme.border,
          },
        ]}
      >
        {checked ? <Ionicons name="checkmark" size={16} color={theme.white} /> : null}
      </View>
...
export function SegmentedControl({ options, value, onChange, style }: SegmentedControlProps) {
  const theme = useAppTheme();
  return (
    <View style={[styles.segmentWrap, { backgroundColor: theme.grayLight }, style]}>
      {options.map((opt) => {
        ...
        return (
          <PressableScale
            ...
            style={[
              styles.segment,
              active && { backgroundColor: theme.white, shadowColor: theme.shadow },
            ]}
          >
            <Text style={[styles.segmentLabel, { color: active ? theme.text : theme.textLight }]}>
              {opt.label}
            </Text>
          </PressableScale>
        );
...
export function Input({ label, error, style, ...rest }: InputProps) {
  ...
  return (
    <View>
      {label ? <Text style={[styles.inputLabel, { color: theme.textLight }]}>{label}</Text> : null}
      <TextInput
        {...rest}
        placeholderTextColor={theme.textLight}
        style={[
          styles.input,
          { color: theme.text, borderColor: error ? theme.danger : theme.border, backgroundColor: theme.white },
          style,
        ]}
      />
      {error ? <Text style={[styles.inputError, { color: theme.danger }]}>{error}</Text> : null}
    </View>
  );
}
