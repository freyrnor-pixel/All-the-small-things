 ...
 *
 * Connections:
 *   Imports → constants/theme, lib/i18n, lib/useAppTheme, store/useCatalogStore, store/useMealStore, react-native-reanimated
 *   Used by → app/shopping.tsx (Monthly tab)
...
import { useScaledStyles, useAccessibility } from '@/lib/useAppTheme';
...
export default function AddDishSheet({ visible, theme, onClose, onSave }: Props) {
        ...
        <Animated.View style={[styles.card, { backgroundColor: theme.white }, cardStyle]}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" style={styles.scrollView}>
            <View style={styles.modeRow}>
              <Pressable
                style={[styles.modePill, { backgroundColor: mode === 'new' ? theme.orange : theme.grayLight }]}
                onPress={() => setMode('new')}
              >
                <Text style={[styles.modePillText, { color: mode === 'new' ? '#fff' : theme.text }]}>{t.newDishToggle}</Text>
              </Pressable>
              <Pressable
                style={[styles.modePill, { backgroundColor: mode === 'fromMeals' ? theme.orange : theme.grayLight }]}
                onPress={() => setMode('fromMeals')}
              >
                <Text style={[styles.modePillText, { color: mode === 'fromMeals' ? '#fff' : theme.text }]}>{t.fromMealsToggle}</Text>
              </Pressable>
            </View>
            ...
            {mode === 'fromMeals' && (
              <>
                ...
                {dishMatches.length === 0 ? (
                  <Text style={[styles.emptyDishesText, { color: theme.textLight }]}>{t.noDishesAvailable}</Text>
                ) : (
                  <View style={[styles.dishPickBox, { backgroundColor: theme.offWhite, borderColor: theme.grayLight }]}>
                    <ScrollView keyboardShouldPersistTaps="handled" style={styles.dishPickScroll}>
                      {dishMatches.map((d) => (
                        <Pressable
                          key={d.id}
                          style={[styles.dishPickRow, dishName === d.name && { backgroundColor: theme.orangeLight }]}
                          onPress={() => pickMealsDish(d.id)}
                        >
                          <Text style={[styles.dishPickName, { color: theme.text }]} numberOfLines={1}>{d.name}</Text>
                          <Text style={[styles.dishPickMeta, { color: theme.textLight }]}>{t.ingredientsCount(d.ingredients.length)}</Text>
                        </Pressable>
                      ))}
                    </ScrollView>
                  </View>
            ...
            <Text style={[styles.label, { color: theme.textLight }]}>{t.dishNamePlaceholder}</Text>
            <TextInput
              style={[styles.input, { backgroundColor: theme.offWhite, color: theme.text }]}
              value={dishName}
            ...
            <View style={styles.ingAddRow}>
              <TextInput
                style={[styles.priceInput, { backgroundColor: theme.offWhite, color: theme.text }]}
                value={ingPrice}
            ...
            <View style={styles.actionsRow}>
              ...
              <Pressable
                style={[styles.primaryBtn, { backgroundColor: theme.orange }, !canSave && { opacity: 0.4 }]}
                onPress={handleSave}
                disabled={!canSave}
              >
                <Text style={styles.primaryBtnText}>{t.addDishBtn}</Text>
              </Pressable>
            </View>
          </ScrollView>
        </Animated.View>
...
const baseStyles = StyleSheet.create({
  ...
  addIngBtnText: { color: '#fff', fontSize: FontSize.lg, fontFamily: Fonts.bold, lineHeight: 22 },
