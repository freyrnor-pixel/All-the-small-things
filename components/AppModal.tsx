 ...
 *
 * Connections:
 *   Imports → components/PressableScale, constants/theme, lib/i18n, lib/useAppTheme, react-native-reanimated
 *   Used by → app/_layout.tsx (host); every screen/store that previously called Alert.alert
 ...
 *     call sites too (store/useAutomationStore.ts fires it from a Zustand action). It talks
 *     to the mounted host through a single module-level listener ref, not React context.
 ...
 *   - Entrance/exit timing follows ANIMATION_GUIDELINES.md §1 (300-350ms ease-out in /
 *     200-250ms ease-in out), gated by useAccessibility().reducedMotion.
 */
...
export default function AppModalHost() {
  ...
  function buttonLook(style?: AppModalButton['style']) {
    if (style === 'cancel') return { fill: 'transparent', textColor: theme.textLight, scaleTo: 0.97 };
    if (style === 'destructive') return { fill: theme.danger, textColor: '#FFFFFF', scaleTo: 0.93 };
    return { fill: theme.orange, textColor: '#FFFFFF', scaleTo: 0.95 };
  }
    ...
    <Modal visible transparent animationType="none" onRequestClose={() => dismiss()}>
      <View style={styles.overlay}>
        ...
        <Animated.View style={[styles.card, { backgroundColor: theme.white }, cardStyle]}>
          ...
          {!!request.message && <Text style={[styles.message, { color: theme.textLight }]}>{request.message}</Text>}
