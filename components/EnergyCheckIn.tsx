 ...
 *   Data    → useEnergyStore (energy_logs table, one row per day)
 *
 * Edit notes:
 ...
 *     "select the raw slice, not just a function ref" gotcha in app/index.tsx.
...
export default function EnergyCheckIn() {
  ...
  return (
    <Surface style={styles.card}>
      <Text style={[styles.prompt, { color: theme.textLight }]}>{t.energy.checkInPrompt}</Text>
          ...
          return (
            <Pressable
              ...
              style={[
                ...
                active && { backgroundColor: theme.orangeLight, borderColor: theme.orange, borderWidth: 1.5 },
              ]}
              onPress={() => {
                selection();
                setToday(level);
      ...
      {todayLevel === 'low' && (
        <Text style={[styles.hint, { color: theme.textLight }]}>{t.energy.lowEnergyHint}</Text>
      )}
    </Surface>
  );
}
