...
//      (resolved appVersion 1.1.0 != embedded build runtime 1.0.0, no error surfaced).
...
if (typeof rv !== 'string' || rv.trim() === '') {
  errors.push(
    ...
    `it resolved to a runtime that did not match the installed APK's embedded runtime, so EVERY OTA update ` +
    `was silently rejected on-device with no error anywhere. Do not change this without also building and ` +
    `shipping a new APK with a matching runtimeVersion — see AGENTS.md "Runtime version" section.`
  );
}
